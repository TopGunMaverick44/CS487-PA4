...
def my_orchestrator(context: df.DurableOrchestrationContext):
    ...
    # If invalid, stop and reject
    if not validation.get("valid"):
        return {"status": "rejected", "reason": validation.get("reason", "unknown")}
    
    # If valid, spawn the Container Instance report job
...
def report_activity(order: dict) -> str:
    ...
    # 1. Define the Container Group parameters
    ...
    # 2. Tell Azure to create it
    client.container_groups.begin_create_or_update(rg, name, group).result()

    # 3. Poll the container every 5 seconds until it finishes generating the PDF
    for _ in range(60):
    ...
    # 4. Delete the container so you don't get charged for a completed job
    client.container_groups.begin_delete(rg, name)

    # 5. Return the URL where the PDF was saved
